...
class GemeloTermico:
    def __init__(self, t_inicial, alpha, beta):
        ...
        self.alpha = alpha  # Coeficiente de acoplamiento con el sensor real
        self.beta = beta    # Factor de ganancia por ocupación interna

    def computar_estado(self, t_real, hora, anomalia=False):
        # Ganancia interna determinista por ocupación (colegios de 8 a 14h)
        q_ocu = 0.15 if (8 <= hora <= 14) else 0.0
        
        # Inyección física de anomalía (ej: rotura de aislamiento / ventana abierta en invierno)
        if anomalia:
            self.alpha = 0.45  # El acoplamiento térmico exterior se dispara brutalmente
            q_ocu = -0.6       # Pérdida masiva de calor por corrientes de aire
            
        # Ecuación en diferencias finitas basada en conducción térmica
        gradiente = t_real - self.t_actual
        self.t_actual += (self.alpha * gradiente) + (self.beta * q_ocu)
        
        # Ruido instrumental blanco Gaussiano mínimo simétrico
...
class GemeloElectrico:
    def __init__(self, p_base, gamma):
        self.p_base = p_base
        self.gamma = gamma  # Eficiencia del climatizador por grado de desviación

    def computar_estado(self, t_int_sim, hora, anomalia=False):
        # Curva de consumo base de la instalación según ciclo operativo diurno
        base_actual = self.p_base * (1.3 if (7 <= hora <= 18) else 0.4)
        
        # Consumo reactivo del compresor HVAC para mantener confort (21°C objetivo)
        desviacion_termica = abs(t_int_sim - 21.0)
        p_hvac = desviacion_termica * self.gamma
        
        if anomalia:
            # Anomalía: Fallo crítico de derivación a tierra o sobrecarga de línea
...
class GemeloHidraulico:
    ...
    def computar_estado(self, hora, anomalia=False):
        # Modelo determinista de demanda por pulsos horarios de uso
        if (8 <= hora <= 9) or (13 <= hora <= 14):
            base = self.caudal_nominal * 0.85
        elif (10 <= hora <= 17):
            base = self.caudal_nominal * 0.25
        else:
            base = 0.0 # Consumo nocturno cero o residual estructural
            
        if anomalia:
            # Anomalía: Fuga franca sostenida en tubería de distribución secundaria
            return round(base + (self.caudal_nominal * 1.5), 2)
            
        # Pequeña fluctuación de presión hidrodinámica
        fluctuacion_presion = ((urandom.getrandbits(8) / 255.0) - 0.5) * 0.15
        return round(max(0.0, base + fluctuacion_presion), 2)

# Instanciación y mapeo de los motores físicos individuales
motores = {
    "col_clima_int_01": GemeloTermico(t_inicial=21.0, alpha=0.05, beta=1.2),
    "hosp_clima_int_01": GemeloTermico(t_inicial=22.5, alpha=0.02, beta=0.8), # Mejor aislado
...
def generar_firma_tuya(method, url_path, token="", body_str=""):
    # Corrección de desfase horario para alinearse estrictamente con Tuya Cloud UTC
        ...
        if js.get("success"):
            for item in js["result"]:
                if item["code"] == "va_temperature":
                    return item["value"] * 0.1  # Escala correcta a grados Celsius
    except:
        return None
    return None
...
def main():
    ...
    # Almacén local temporal para vincular estados térmicos cruzados con potencia
    ...
    while True:
        ...
        # Fallback de seguridad matemática si falla la llamada HTTP a Tuya
        ...
        # ==============================================================================
        # --- AÑADIDO: ENVÍO DEL SENSOR REAL A NODE-RED PARA COMPARATIVA EN GRAFANA ---
        # ==============================================================================
        payload_real = {
            "metadatos": {
        ...
        for sensor in INVENTARIO_SENSORES:
            ...
            # --- MOTOR DE INYECCIÓN DE ANOMALÍAS ETIQUETADAS (GROUND TRUTH) ---
            # Probabilidad exacta controlada del 6% por ciclo
            forzar_falla = (urandom.getrandbits(8) % 100) < 6
            tipo_falla = "NOMINAL"

            valor_calculado = 0.0

            # Computación a través de las ecuaciones diferenciales de los gemelos
            if tipo == "temperatura_interior":
                ...
                # La temperatura exterior simula la fachada norte del edificio, suavizada respecto al conductor
                ...
                # Acoplamiento físico cruzado: requiere la temperatura interior simulada de su centro
            ...
            # --- ESTRUCTURACIÓN ACADÉMICA CIENTÍFICA DEL JSON ---
            payload = {
                "metadatos": sensor,
                "timestamp_local": timestamp_epoch,
            ...
            try:
                ...
                print(f"🔬 [Consola Local] {id_disp} -> {mensaje_json}")
