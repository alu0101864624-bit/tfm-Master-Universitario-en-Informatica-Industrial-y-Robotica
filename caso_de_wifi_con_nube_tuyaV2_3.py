...
# --- 2. CONFIGURACIÓN MQTT Y ARQUITECTURA MULTI-INSTALACIÓN ---
...
# 🌟 DICCIONARIO DE METADATOS: Cambia este bloque en cada ESP32 según dónde lo instales
METADATOS_NODO = {
    "isla": "Tenerife",
    "municipio": "La_Laguna",
    "tipo_instalacion": "Colegio",
    "centro": "San_Cristobal",
    "zona": "Aula_Principal"
}

# Construcción dinámica del Tópico MQTT (Sigue un árbol jerárquico limpio)
# Ejemplo: canarias/Colegio/Tenerife/La_Laguna/San_Cristobal/datos
MQTT_TOPIC = "canarias/{}/{}/{}/{}/datos".format(
    METADATOS_NODO["tipo_instalacion"],
    METADATOS_NODO["isla"],
    METADATOS_NODO["municipio"],
    METADATOS_NODO["centro"]
).encode('utf-8')
    ...
    for servidor in servidores_ntp:
        ...
        for intento in range(3):
            try:
                ...
                time.sleep(1.5)
    print("❌ ERROR: No se pudo avanzar hora por internet.")
...
# ==========================================
# LÓGICA DE SENSORES SIMULADOS (PASEO ALEATORIO)
# ==========================================
estado_simulacion = {
    "temp_int": 21.0,
    "hum_int": 55.0,
...
# ==========================================
def main():
    print("\n⚡ INICIANDO NODO IOT MULTI-INSTALACIÓN DISTRIBUIDO ⚡")
    conectar_wifi()
    cliente_mqtt = conectar_mqtt()
    
    while True:
        gc.collect()
        print(f"\n[{time.ticks_ms()}] Recolectando datos...")
        ...
        if token:
            ...
            if datos_reales_interior:
                ...
                # 📦 MACRO-JSON CON SUB-OBJETO DE METADATOS COMPLETO
                payload_empaquetado = {
                    "metadatos": METADATOS_NODO,
                    # Mantengo procedencia combinada por compatibilidad en Node-RED
                    "procedencia": "{}_{}".format(METADATOS_NODO["centro"], METADATOS_NODO["zona"]),
                ...
                print("📦 Payload generado para [{}]:".format(payload_empaquetado["procedencia"]), mensaje_json)
                print("📡 Publicando en Tópico:", MQTT_TOPIC.decode('utf-8'))
                
                if cliente_mqtt:
                    try:
                        cliente_mqtt.publish(MQTT_TOPIC, mensaje_json.encode('utf-8'))
                        print("🚀 Paquete distribuido con éxito.")
                    except:
                        print("⚠️ Conexión MQTT perdida. Reconectando...")
                        cliente_mqtt = conectar_mqtt()
            else:
                print("❌ Fallo al capturar datos reales de Tuya.")
        
        print(f"⏳ Esperando {INTERVALO_PRUEBAS_SEG} segundos...")
